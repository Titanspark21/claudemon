...
test('the battle theme ships, and ships as something every player takes', () => {
  // Unlike a blip, this one is a file in the repo rather than something computed, so
  // the test is whether the install is intact and whether what shipped is the format
  // the players in sound.mjs can actually open. An mp3 here would be silence on Linux
  // without ffplay and silence on Windows, and nothing else would notice.
  assert.ok(existsSync(MUSIC.battle.file), `no track at ${MUSIC.battle.file}`)

  const wav = readFileSync(MUSIC.battle.file)
  const header = readHeader(wav)

  assert.equal(header.riff, 'RIFF')
  assert.equal(header.wave, 'WAVE')
  assert.equal(header.data, 'data', 'the data chunk is not where a 44-byte header puts it')
  assert.equal(header.format, 1, 'plain PCM')
  assert.equal(header.channels, 1)
  assert.equal(header.bits, 16)
  assert.equal(header.dataSize, wav.length - 44)
  ...
  // Long enough that a battle rarely reaches the seam, and short enough that it is a
  // file in a repo rather than a download.
  assert.ok(seconds > 30, `runs ${seconds.toFixed(1)}s, which is a jingle`)
  assert.ok(seconds < 180, `runs ${seconds.toFixed(1)}s, which is a lot of WAV`)
})

test('the battle theme is audible without being louder than the blips', () => {
  const wav = readFileSync(MUSIC.battle.file)
  ...
  assert.ok(rms > 0.02, `RMS ${rms.toFixed(4)} is close enough to silence to look broken`)
  assert.ok(rms < 0.2, `RMS ${rms.toFixed(4)} is loud next to a blip at gain 0.2`)
  assert.ok(peak < 0.99, `peaks at ${peak.toFixed(3)}, which is clipping or close to it`)
})
