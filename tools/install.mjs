...
const binTarget = join(homedir(), '.local', 'bin', 'claudemon')
const settingsPath = join(homedir(), '.claude', 'settings.json')

/** The status line's entry point: a shim, for the reason given on {@link shimSource}. */
const statusLineShim = join(HOME, 'statusline.sh')
const statusLineCommand = `"${statusLineShim}"`

/** Where Claude Code keeps the copy of a plugin it installed. */
const PLUGIN_CACHE = join(homedir(), '.claude', 'plugins', 'cache', 'claudemon', 'claudemon')

const uninstalling = process.argv.includes('--uninstall')

/**
 * A launcher that finds claudemon's files at run time rather than being told once.
 *
 * Installing the plugin copies it to a directory with the version in the path, so
 * anything pointing straight at that breaks on the next release — the command
 * disappears from your PATH and the status line goes blank, both silently. So the
 * two things that outlive an upgrade go through one of these instead.
 *
 * The path this was installed from is tried first, which is what makes a clone work
 * for development, and the newest plugin copy is the fallback. Newest by mtime, not
 * by version order: `sort -V` is not portable, and the directory written most
 * recently is the one just installed.
 */
function shimSource(target, args = '') {
  return `#!/bin/sh
# Generated by claudemon's installer — rerun it rather than editing this.
#
# Resolves where claudemon lives, then hands over. See tools/install.mjs.

app="${projectRoot}"
[ -d "$app" ] || app=$(ls -td "${PLUGIN_CACHE}"/*/ 2>/dev/null | head -1)
...
/** Writes an executable shim, creating its directory. */
function writeShim(path, target, args) {
  mkdirSync(dirname(path), { recursive: true })
  writeFileSync(path, shimSource(target, args))
...
function installCommand() {
  ...
  writeShim(binTarget, 'bin/claudemon')
  writeShim(statusLineShim, 'scripts/run.sh', 'statusline.mjs')
...
function removeCommand() {
  let removed = false
  for (const path of [binTarget, statusLineShim]) {
